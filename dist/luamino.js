...
class Scope {
    ...
    constructor(parent = null) {
        ...
        if (parent) {
            for (let name of parent.usedNames)
    ...
    getLocalName(originalName, isLocal) {
        ...
        let scope = this;
        while (scope) {
            if (scope.locals.has(originalName))
                return scope.locals.get(originalName);
            scope = scope.parent;
        }
...
function joinStatements(a, b, separator) {
    separator = separator || ' ';
    if (a === '' || a === undefined)
        return b;
    if (b === '' || b === undefined)
    ...
    if (lastCharA === '-' && firstCharB === '-')
        return a + separator + b;
    if (lastCharA === '.' && a.slice(-2, -1) !== '.' && regexAlphaNumUnderscore.test(firstCharB))
        return a + separator + b;
    return a + b;
}
// main minifier class
class Minifier {
    ...
    constructor(globals = []) {
        ...
        for (const g of globals)
            this.globalScope.reserveName(g.name);
    ...
    exitScope() { this.scopeStack.shift(); }
    currentScope() {
    ...
    walkStatement(stmt) {
        switch (stmt.type) {
            case 'AssignmentStatement':
                const lhs = stmt.variables.map((v) => this.walkExpression(v)).join(',');
                const rhs = stmt.init.map((i) => this.walkExpression(i)).join(',');
                return `${lhs}=${rhs}`;
            case 'LocalStatement':
                const names = stmt.variables.map((v) => this.currentScope().getLocalName(v.name, true)).join(',');
                if (stmt.init.length) {
                    const vals = stmt.init.map((i) => this.walkExpression(i)).join(',');
            ...
            case 'FunctionDeclaration':
                ...
                for (const p of stmt.parameters) {
                    if (p.type === 'Identifier')
                        params.push(this.currentScope().getLocalName(p.name, true));
            ...
            case 'ForGenericStatement':
                this.enterScope();
                const vars = stmt.variables.map((v) => this.currentScope().getLocalName(v.name, true)).join(',');
                const iters = stmt.iterators.map((i) => this.walkExpression(i)).join(',');
    ...
    walkExpression(expr, options = { precedence: 0, direction: 'left', parent: null }) {
        switch (expr.type) {
            case 'Identifier':
                for (const scope of this.scopeStack) {
                    if (scope.locals.has(expr.name))
                        return scope.locals.get(expr.name);
                }
                return expr.name;
            case 'StringLiteral':
            case 'NumericLiteral':
            ...
            case 'FunctionDeclaration':
                ...
                for (const p of expr.parameters) {
                    if (p.type === 'Identifier')
                        fparams.push(this.currentScope().getLocalName(p.name, true));
                    else if (p.type === 'VarargLiteral')
                        fparams.push('...');
                }
...
export { minify };
//# sourceMappingURL=luamino.js.map